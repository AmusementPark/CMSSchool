...
public class News extends BaseModel<News> {
    ...
    public Page getNewsByIndex(int index, int bk, int pageNumber){
		...
		param[0] = index;
		param[1] = bk;
		Page page = this.paginate(pageNumber, DrwConstants.pageSize, "select * ", " from sch_news where news_index=? and news_bankuai=? order by news_topic_top desc, news_time desc", param);
		return page;
	}
